package fi.jyu.it.ties456.week38.services.web;
...
@Title("Vaadin SOAP client")
public class MainView extends UI	 {

	/* User interface components are stored in session. */
	private Table contactList = new Table();
	private TextField searchField = new TextField();
	private Button addNewContactButton = new Button("create");
	private Button createCourseButton = new Button("create a course");
	...
	private static final String[] fieldNames = new String[] { FNAME, LNAME,
			ID, EMAIL, COURSENAME,CREDITS,DESCRIPTION};
//			, "Mobile Phone", "Work Phone", "Home Phone", "Work Email","Home Email", "Street", "City", "Zip", "State", "Country" };
...
//		bottomLeftLayout.addComponent(addNewContactButton);
	...
	private void initEditor() {
		...
		editorLayout.addComponent(createCourseButton);
	...
	private void initAddCreateButtons() {
		...
		createCourseButton.addClickListener(new ClickListener() {
	            public void buttonClick(ClickEvent event) {
//	                    Object contactId = contactList.getValue();
//	                    contactList.removeItem(contactId);
	            	StudentISService createTeacherInfo= new StudentISService();
					StudentIS getConnected2= createTeacherInfo.getStudentISPort();
					Object contactId = contactList.getValue();
					String userId = (String)contactList.getContainerProperty(contactId, ID).getValue();
					String courseName = (String)contactList.getContainerProperty(contactId, COURSENAME).getValue();
					String credit = (String)contactList.getContainerProperty(contactId, CREDITS).getValue();
					int creditsInt = Integer.parseInt(credit);
					String description = (String)contactList.getContainerProperty(contactId, DESCRIPTION).getValue();
					String courseId = getConnected2.createCourse(courseName, userId, creditsInt, description);
					Notification.show(courseId);
	            }
	    });
}
	...
	private void initContactList() {
		...
		contactList.setVisibleColumns(new String[] { FNAME, LNAME, ID, EMAIL, COURSENAME,CREDITS, DESCRIPTION });
	...
	private static IndexedContainer createDummyDatasource() {
		...
		PeopleRegistryService getPeopleService= new PeopleRegistryService();
		PeopleRegistry registry = getPeopleService.getPeopleRegistryPort();
		  List<PersonInfoType> people = registry.searchForPerson("");
		  
		  for (PersonInfoType type : people) {
